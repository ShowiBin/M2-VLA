...
class VisionBackbone(nn.Module, ABC):
    ...
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int=224, image_sequence_len: int=1) -> None:
        super().__init__()
...
class TimmViTBackbone(VisionBackbone, ABC):
    ...
    def __init__(self, vision_backbone_id: str, timm_path_or_url: str, image_resize_strategy: str, default_image_size: int=224, image_sequence_len: int=1, override_act_layer: Optional[str]=None) -> None:
        ...
        if self.override_act_layer is None:
            self.featurizer: VisionTransformer = timm.create_model(self.timm_path_or_url, pretrained=True, num_classes=0, img_size=self.default_image_size)
        else:
            self.featurizer: VisionTransformer = timm.create_model(self.timm_path_or_url, pretrained=True, num_classes=0, img_size=self.default_image_size, act_layer=self.override_act_layer)
        self.featurizer.eval()
        self.featurizer.forward = unpack_tuple(partial(self.featurizer.get_intermediate_layers, n={len(self.featurizer.blocks) - 2}))
        assert isinstance(self.featurizer, VisionTransformer), 'Featurizer is not a TIMM VisionTransformer; if you would like to support a new visual representation, file an issue or implement the requisite logic (see `prismatic/models/backbones/vision/base_vision.py`)!'
        ...
        if 'siglip' in self.timm_path_or_url or 'in1k' in self.timm_path_or_url:
            ...
            default_image_transform = Compose([Resize(self.default_image_size, interpolation=default_image_transform.transforms[0].interpolation), *default_image_transform.transforms[1:]])
        if self.image_resize_strategy == 'resize-naive':
            assert isinstance(default_image_transform, Compose), 'Unexpected `default_image_transform`!'
            assert isinstance(default_image_transform.transforms[0], Resize)
            target_size = (self.default_image_size, self.default_image_size)
            self.image_transform = Compose([Resize(target_size, interpolation=default_image_transform.transforms[0].interpolation), *default_image_transform.transforms[1:]])
    ...
    def forward(self, pixel_values: Union[torch.Tensor, Dict[str, torch.Tensor]]) -> torch.Tensor:
        if self.image_sequence_len == 1:
            ...
            return compute_sequence_patches({'img': pixel_values}, {'img': self.featurizer}, self.image_sequence_len)['img']
