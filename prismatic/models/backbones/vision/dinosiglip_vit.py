...
from prismatic.models.backbones.vision.base_vision import ImageTransform, LetterboxPad, VisionBackbone, compute_sequence_patches, unpack_tuple
DINOSigLIP_VISION_BACKBONES = {'dinosiglip-vit-so-224px': {'dino': 'vit_large_patch14_reg4_dinov2.lvd142m', 'siglip': 'vit_so400m_patch14_siglip_224'}, 'dinosiglip-vit-so-384px': {'dino': 'vit_large_patch14_reg4_dinov2.lvd142m', 'siglip': 'vit_so400m_patch14_siglip_384'}}
...
class DinoSigLIPViTBackbone(VisionBackbone):
    ...
    def __init__(self, vision_backbone_id: str, image_resize_strategy: str, default_image_size: int=224, image_sequence_len: int=1) -> None:
        ...
        self.dino_featurizer: VisionTransformer = timm.create_model(self.dino_timm_path_or_url, pretrained=True, num_classes=0, img_size=self.default_image_size)
        self.dino_featurizer.eval()
        self.siglip_featurizer: VisionTransformer = timm.create_model(self.siglip_timm_path_or_url, pretrained=True, num_classes=0, img_size=self.default_image_size)
        ...
        default_siglip_transform = Compose([Resize(self.default_image_size, interpolation=default_siglip_transform.transforms[0].interpolation), *default_siglip_transform.transforms[1:]])
        if self.image_resize_strategy == 'resize-naive':
            ...
            assert 'mean' in self.dino_data_cfg and 'mean' in self.siglip_data_cfg, 'DinoSigLIP `data_cfg` missing `mean`!'
            dino_fill = tuple([int(x * 255) for x in self.dino_data_cfg['mean']])
            siglip_fill = tuple([int(x * 255) for x in self.siglip_data_cfg['mean']])
            self.image_transform = DinoSigLIPImageTransform(Compose([LetterboxPad(dino_fill), *default_dino_transform.transforms]), Compose([LetterboxPad(siglip_fill), *default_siglip_transform.transforms]))
        else:
