...
class TransFuser(nn.Module):
    ...
    def __init__(self, vis_channels: int, depth_channels: int, tgt_channels: int, num_heads: int) -> None:
        super().__init__()
        self.attn = CrossAttentionBlock(v_dim=tgt_channels, l_dim=tgt_channels, embed_dim=tgt_channels, num_heads=num_heads)
...
class MAPBlock(nn.Module):
    ...
    def __init__(self, n_latents: int, vis_dim: int, embed_dim: int, n_heads: int, mlp_ratio: float=4.0, do_rms_norm: bool=True, do_swish_glu: bool=True) -> None:
        super().__init__()
        ...
        self.attn_norm = RMSNorm(self.embed_dim) if do_rms_norm else nn.LayerNorm(self.embed_dim, eps=1e-06)
        self.attn = MAPAttention(self.embed_dim, n_heads=self.n_heads)
        self.mlp_norm = RMSNorm(self.embed_dim) if do_rms_norm else nn.LayerNorm(self.embed_dim, eps=1e-06)
        self.mlp = nn.Sequential(SwishGLU(self.embed_dim, int(mlp_ratio * self.embed_dim)) if do_swish_glu else nn.Sequential(nn.Linear(self.embed_dim, int(mlp_ratio * self.embed_dim)), nn.GELU()), nn.Linear(int(mlp_ratio * self.embed_dim), self.embed_dim))
...
class CrossAttentionBlock(nn.Module):
    ...
    def __init__(self, v_dim, l_dim, embed_dim, num_heads, dropout=0.1, drop_path=0.0, init_values=0.0001, cfg=None, RoPE=False):
        super(CrossAttentionBlock, self).__init__()
        self.layer_norm_v = nn.LayerNorm(v_dim)
        self.layer_norm_l = nn.LayerNorm(l_dim)
        self.attn = CrossAttention(v_dim=v_dim, l_dim=l_dim, embed_dim=embed_dim, num_heads=num_heads, dropout=dropout, RoPE=RoPE)
...
class PerceiverResampler(nn.Module):
    ...
    def __init__(self, *, dim, vis_dim, depth, dim_head=64, heads=8, num_latents=64, num_media_embeds=4, ff_mult=4):
        super().__init__()
