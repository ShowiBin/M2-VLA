...
class WeightsBiasesTracker:
    ...
    def __init__(self, run_id: str, run_dir: Path, hparams: Dict[str, Any], project: str='prismatic', entity: Optional[str]=None, group: str='align') -> None:
        (self.run_id, self.run_dir, self.hparams) = (run_id, run_dir, hparams)
    ...
    def initialize(self) -> None:
        wandb.init(name=self.run_id, dir=self.wandb_dir, config=self.hparams, project=self.project, entity=self.entity, group=self.group)
...
class Metrics:
    ...
    def __init__(self, active_trackers: Tuple[str, ...], run_id: str, run_dir: Path, hparams: Dict[str, Any], stage: str, wandb_project: str='prismatic', wandb_entity: Optional[str]=None, grad_accumulation_steps: int=1, window_size: int=128) -> None:
        ...
        self.state = {'loss_raw': deque(maxlen=grad_accumulation_steps), 'loss': deque(maxlen=window_size), 'step_time': deque(maxlen=window_size), 'lr': []}
    ...
    def push(self) -> str:
        ...
        self.log(self.global_step, metrics={f'{prefix}/Step': self.global_step, f'{prefix}/Loss': loss, f'{prefix}/Loss (Raw)': loss_raw, f'{prefix}/Learning Rate': lr, f'{prefix}/Step Time': step_time})
        return status
...
class VLAMetrics:
    ...
    def __init__(self, active_trackers: Tuple[str, ...], run_id: str, run_dir: Path, hparams: Dict[str, Any], wandb_project: str='openvla', wandb_entity: Optional[str]='stanford-voltron', grad_accumulation_steps: int=1, window_size: int=1, resume_step: Optional[int]=None, resume_epoch: Optional[int]=None) -> None:
        (self.run_id, self.run_dir, self.hparams) = (run_id, run_dir, hparams)
        self.trackers = []
        for tracker_type in active_trackers:
            if tracker_type == 'jsonl':
        ...
        self.state = {'loss_raw': deque(maxlen=grad_accumulation_steps), 'loss': deque(maxlen=window_size), 'l1_loss': deque(maxlen=window_size), 'action_accuracy': deque(maxlen=window_size), 'step_time': deque(maxlen=window_size), 'lr': []}
    ...
    def commit(self, *, global_step: Optional[int]=None, epoch: Optional[int]=None, lr: Optional[float]=None, update_step_time: bool=False, **kwargs) -> None:
        if global_step is not None:
            self.global_step = global_step
    ...
    def push(self) -> str:
        ...
        for (ds, tracker) in self.dataset_trackers.items():
            dataset_metrics.update({f'{ds}/L1 Loss': torch.stack(list(tracker.state['l1_loss'])).mean().item(), f'{ds}/Action Token Accuracy': torch.stack(list(tracker.state['action_accuracy'])).mean().item()})
        prefix = 'VLA Train'
        self.log(self.global_step, metrics={f'{prefix}/Step': self.global_step, f'{prefix}/Epoch': self.epoch, f'{prefix}/Loss': loss, f'{prefix}/L1 Loss': l1_loss, f'{prefix}/Action Token Accuracy': action_accuracy, f'{prefix}/Loss (Raw)': loss_raw, f'{prefix}/Learning Rate': lr, f'{prefix}/Step Time': step_time, **dataset_metrics})
        return status
