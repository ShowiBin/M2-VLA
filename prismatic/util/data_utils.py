...
class PaddedCollatorForLanguageModeling:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        if len(multimodal_indices) == 0:
            ...
            pixel_values = {k: torch.stack([pixel_values[idx][k] if idx in multimodal_indices else self.dummy_pixel_values for idx in range(len(input_ids))]) for k in pv_example}
        else:
            raise ValueError(f'Unsupported `pixel_values` type = {type(pixel_values)}')
        return dict(pixel_values=pixel_values, input_ids=input_ids, attention_mask=attention_mask, labels=labels, multimodal_indices=multimodal_indices)
...
class PaddedCollatorForActionPrediction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        if 'dataset_name' in instances[0]:
            ...
            dataset_names = None
        assert self.padding_side == 'right', f'Invalid Tokenizer `self.padding_side = {self.padding_side!r}`'
        input_ids = pad_sequence(input_ids, batch_first=True, padding_value=self.pad_token_id)
        labels = pad_sequence(labels, batch_first=True, padding_value=IGNORE_INDEX)
        ...
        if 'proprio' in instances[0]:
            ...
            proprio = None
        output = dict(pixel_values=pixel_values, proprio=proprio, input_ids=input_ids, attention_mask=attention_mask, labels=labels, actions=actions)
