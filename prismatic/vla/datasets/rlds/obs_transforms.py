...
def augment(obs: Dict, seed: tf.Tensor, augment_kwargs: Union[Dict, Dict[str, Dict]]) -> Dict:
    ...
    for (i, name) in enumerate(image_names):
        ...
        obs[f'image_{name}'] = tf.cond(obs['pad_mask_dict'][f'image_{name}'], lambda : dl.transforms.augment_image(obs[f'image_{name}'], **kwargs, seed=seed + i), lambda : obs[f'image_{name}'])
    return obs

def decode_and_resize(obs: Dict, resize_size: Union[Tuple[int, int], Dict[str, Tuple[int, int]]], depth_resize_size: Union[Tuple[int, int], Dict[str, Tuple[int, int]]]) -> Dict:
    image_names = {key[6:] for key in obs if key.startswith('image_')}
    ...
    for name in image_names:
        if name not in resize_size:
            logging.warning(f'No resize_size was provided for image_{name}. This will result in 1x1 padding images, which may cause errors if you mix padding and non-padding images.')
    ...
    for name in depth_names:
        if name not in depth_resize_size:
            logging.warning(f'No depth_resize_size was provided for depth_{name}. This will result in 1x1 padding depth images, which may cause errors if you mix padding and non-padding images.')
