...
def normalize_action_and_proprio(traj: Dict, metadata: Dict, normalization_type: NormalizationType):
    ...
    if normalization_type == NormalizationType.NORMAL:
        for (key, traj_key) in keys_to_normalize.items():
            ...
            traj = dl.transforms.selective_tree_map(traj, match=lambda k, _: k == traj_key, map_fn=lambda x: tf.where(mask, (x - metadata[key]['mean']) / (metadata[key]['std'] + 1e-08), x))
        return traj
        ...
        for (key, traj_key) in keys_to_normalize.items():
            ...
            traj = dl.transforms.selective_tree_map(traj, match=lambda k, _: k == traj_key, map_fn=lambda x: tf.where(mask, tf.clip_by_value(2 * (x - low) / (high - low + 1e-08) - 1, -1, 1), x))
...
def get_dataset_statistics(dataset: dl.DLataset, hash_dependencies: Tuple[str, ...], save_dir: Optional[str]=None) -> Dict:
    ...
    if os.path.exists(local_path):
        ...
        with open(local_path, 'r') as f:
            metadata = json.load(f)
        return metadata
    dataset = dataset.traj_map(lambda traj: {'action': traj['action'], 'proprio': traj['observation']['proprio'] if 'proprio' in traj['observation'] else tf.zeros_like(traj['action'])})
    ...
    metadata = {'action': {'mean': actions.mean(0).tolist(), 'std': actions.std(0).tolist(), 'max': actions.max(0).tolist(), 'min': actions.min(0).tolist(), 'q01': np.quantile(actions, 0.01, axis=0).tolist(), 'q99': np.quantile(actions, 0.99, axis=0).tolist()}, 'proprio': {'mean': proprios.mean(0).tolist(), 'std': proprios.std(0).tolist(), 'max': proprios.max(0).tolist(), 'min': proprios.min(0).tolist(), 'q01': np.quantile(proprios, 0.01, axis=0).tolist(), 'q99': np.quantile(proprios, 0.99, axis=0).tolist()}, 'num_transitions': num_transitions, 'num_trajectories': num_trajectories}
    try:
