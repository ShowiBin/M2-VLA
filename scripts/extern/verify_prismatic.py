...
if '-prism-' in MODEL_PATH:
    SAMPLE_PROMPTS_FOR_GENERATION = ['In: What is sitting in the coffee?\nOut:', "In: What's the name of the food on the plate?\nOut:", 'In: caption.\nOut:', 'In: how many beinets..?\nOut:', 'In: Can you give me a lyrical description of the scene\nOut:']
else:
    SYSTEM_PROMPT = "A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions."
    SAMPLE_PROMPTS_FOR_GENERATION = [f'{SYSTEM_PROMPT} USER: What is sitting in the coffee? ASSISTANT:', f"{SYSTEM_PROMPT} USER: What's the name of the food on the plate? ASSISTANT:", f'{SYSTEM_PROMPT} USER: caption. ASSISTANT:', f'{SYSTEM_PROMPT} USER: how many beinets..? ASSISTANT:', f'{SYSTEM_PROMPT} USER: Can you give me a lyrical description of the scene ASSISTANT:']
...
def verify_prismatic() -> None:
    ...
    vlm = AutoModelForVision2Seq.from_pretrained(MODEL_PATH, attn_implementation='flash_attention_2', torch_dtype=torch.bfloat16, low_cpu_mem_usage=True, trust_remote_code=True).to(device)
    ...
    print(f'[*] Generated Tokens per Second = {num_tokens / total_time} w/ num_tokens = {num_tokens!r} and total_time = {total_time!r}')
