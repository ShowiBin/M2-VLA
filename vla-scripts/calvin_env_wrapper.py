...
class CalvinEnvWrapperRaw(gym.Wrapper):
    ...
    def set_egl_device(device):
        ...
        try:
            ...
            logger.warning("Couldn't find correct EGL device. Setting EGL_VISIBLE_DEVICE=0. When using DDP with many GPUs this can lead to OOM errors. Did you install PyBullet correctly? Please refer to calvin env README")
            egl_id = 0
        os.environ['EGL_VISIBLE_DEVICES'] = str(egl_id)
        logger.info(f'EGL_DEVICE_ID {egl_id} <==> CUDA_DEVICE_ID {cuda_id}')

    def step(self, action_tensor: torch.Tensor) -> Tuple[Dict[str, Union[torch.Tensor, Dict[str, torch.Tensor]]], int, bool, Dict]:
    ...
    def reset(self, reset_info: Dict[str, Any]=None, batch_idx: int=0, seq_idx: int=0, scene_obs: Any=None, robot_obs: Any=None) -> Dict[str, Union[torch.Tensor, Dict[str, torch.Tensor]]]:
        if reset_info is not None:
            obs = self.env.reset(robot_obs=reset_info['robot_obs'][batch_idx, seq_idx], scene_obs=reset_info['scene_obs'][batch_idx, seq_idx])
